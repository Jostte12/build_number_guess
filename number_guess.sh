...
DISPLAY() {
  ...
  if [[ $USER_ID ]]; then
    ...
    #get best game (guess)
    BEST_GUESS=$($PSQL "select min(guesses) from games where player_id = '$USER_ID'")
    ...
    INSERTED_TO_players=$($PSQL "insert into players(name) values('$USERNAME')")
    #get user_id
    USER_ID=$($PSQL "select player_id from players where name = '$USERNAME'")
    # echo $USER_ID
  fi
...
GAME() {
  ...
  while [[ $GUESSED = 0 ]]; do
    ...
    if [[ ! $GUESS =~ ^[0-9]+$ ]]; then
      ...
      #insert into db
      INSERTED_TO_GAMES=$($PSQL "insert into games(player_id, guesses) values($USER_ID, $TRIES)")
      GUESSED=1
    #if greater
